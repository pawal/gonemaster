import { listDomains, type DomainView, type ListResponse } from "$lib/api";
import { filterFromURL } from "$lib/filters";

const DEFAULT_LIMIT = 50;

export type DomainsPageData = {
  datasetTag: string | null;
  list: ListResponse<DomainView> | null;
  error: string | null;
  limit: number;
  offset: number;
};

export async function load({ parent, fetch, url }): Promise<DomainsPageData> {

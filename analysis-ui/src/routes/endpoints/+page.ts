import { listEndpoints, type EndpointView, type ListResponse } from "$lib/api";
import { filterFromURL } from "$lib/filters";

const DEFAULT_LIMIT = 50;

export type EndpointsPageData = {
  datasetTag: string | null;
  list: ListResponse<EndpointView> | null;
  error: string | null;
  limit: number;
  offset: number;
};

export async function load({ parent, fetch, url }): Promise<EndpointsPageData> {

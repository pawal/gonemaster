import { listNameservers, type ListResponse, type NameserverView } from "$lib/api";
import { filterFromURL } from "$lib/filters";

const DEFAULT_LIMIT = 50;

export type NameserversPageData = {
  datasetTag: string | null;
  list: ListResponse<NameserverView> | null;
  error: string | null;
  limit: number;
  offset: number;
};

export async function load({ parent, fetch, url }): Promise<NameserversPageData> {
  ...
  try {
    const list = await listNameservers(

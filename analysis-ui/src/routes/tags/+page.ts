import { listTags, type ListResponse, type TagView } from "$lib/api";
...
export type TagsPageData = {
  ...
  list: ListResponse<TagView> | null;
  error: string | null;
  limit: number;
  offset: number;
};

export async function load({ parent, fetch, url }): Promise<TagsPageData> {

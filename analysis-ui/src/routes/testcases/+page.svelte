<script lang="ts">
  ...
  import type { TestcaseView } from "$lib/api";
  import type { LayoutData } from "../+layout";
  import type { TestcasesPageData } from "./+page";

  let { data }: { data: TestcasesPageData } = $props();
  ...
  const exportColumns: ExportColumn<TestcaseView>[] = [
    ...
    { key: "entry_count", label: "Entries", value: (r) => r.entry_count },
    { key: "unique_tags", label: "Unique tags", value: (r) => r.unique_tags }
  ];

  function filenamePrefix(): string {
    const tag = data.datasetTag ?? "cohort";
    return `${tag}-testcases`;
  }
...
<section class="card">
  ...
  {#if !data.datasetTag}
    <p class="status-banner warn">No cohort resolved. Configure a public cohort to see testcases.</p>
  {:else if data.error}
    <p class="status-banner error">Failed to load testcases: {data.error}</p>
  {:else if !data.list || data.list.items.length === 0}
    <p class="status-banner">No testcases materialized for this cohort yet.</p>
  {:else}
        ...
        <tbody>
          {#each rows as row (`${row.module}/${row.testcase}`)}
            <tr class="row-clickable" onclick={(e: MouseEvent) => rowClick(e, testcaseHref(base, row.testcase, row.module, search))}>
              ...
              <td class="col-num">{formatCount(row.entry_count)}</td>
              <td class="col-num">{formatCount(row.unique_tags)}</td>
            </tr>
          {/each}
        </tbody>
      </table>

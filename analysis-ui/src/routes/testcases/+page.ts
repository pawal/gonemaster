import { listTestcases, type ListResponse, type TestcaseView } from "$lib/api";
import { filterFromURL } from "$lib/filters";

const DEFAULT_LIMIT = 50;

export type TestcasesPageData = {
  datasetTag: string | null;
  list: ListResponse<TestcaseView> | null;
  error: string | null;
  limit: number;
  offset: number;
};

export async function load({ parent, fetch, url }): Promise<TestcasesPageData> {

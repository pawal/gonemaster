...
func applyEnvVars(cfg *server.Config, flagsSet map[string]bool, getenv func(string) string, warn io.Writer) {
	...
	applyBool("auto-clamp-concurrency", "GONEMASTER_AUTO_CLAMP_CONCURRENCY", &cfg.AutoClampConcurrency)
	applyInt("job-test-parallelism", "GONEMASTER_JOB_TEST_PARALLELISM", &cfg.JobTestParallelism)
	...
	applyInt("cross-job-hot-cache-ttl-seconds", "GONEMASTER_CROSS_JOB_HOT_CACHE_TTL_SECONDS", &cfg.CrossJobHotCacheTTLSeconds)

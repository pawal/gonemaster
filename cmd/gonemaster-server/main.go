...
func run(args []string, out *os.File, errOut *os.File) int {
	...
	fs.Usage = func() {
		...
		printUsageGroup(errOut, "Concurrency", []usageLine{
			...
			{flag: "--auto-clamp-concurrency", detail: "Clamp pathological concurrency values based on CPU count"},
			{flag: "--job-test-parallelism N", detail: "Parallel testcases inside one job (default 1)"},
			{flag: "--max-concurrent-jobs N", detail: "Max concurrent engine runs (0 = unlimited)"},
			{flag: "--cross-job-hot-cache", detail: "Enable short-lived warmed nameserver cache reuse across jobs"},
			{flag: "--cross-job-hot-cache-ttl-seconds N", detail: "TTL for hot-cache entries (default 60)"},
		})
	...
	fs.IntVar(&crossJobHotCacheTTLSeconds, "cross-job-hot-cache-ttl-seconds", 0, "TTL for hot-cache entries in seconds (default 60)")
	...
	if flagsSet["job-test-parallelism"] && jobTestParallelism < 1 {
	...
	if flagsSet["cross-job-hot-cache-ttl-seconds"] && crossJobHotCacheTTLSeconds < 1 {
	...
	if flagsSet["auto-clamp-concurrency"] {
		cfg.AutoClampConcurrency = autoClampConcurrency
	}
	if flagsSet["job-test-parallelism"] {
	...
	if flagsSet["cross-job-hot-cache-ttl-seconds"] {

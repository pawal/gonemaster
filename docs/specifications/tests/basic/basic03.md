...
- Preconditions:
  ...
  - Delegation nameserver addresses can be loaded using [`GlueNameservers`](../../nameserver-resolution.md#gluenameservers).
- Required inputs:
...
- If [`GlueNameservers`](../../nameserver-resolution.md#gluenameservers) yields no nameserver addresses, function path emits only testcase markers unless orchestration emitted `HAS_NAMESERVER_NO_WWW_A_TEST`.

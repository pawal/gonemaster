...
- Required inputs:
  ...
  - Recursive lookup function (`recurse`) for non-in-domain NS names.
- Profile/config knobs that affect behavior:
  - `net.ipv4` and `net.ipv6`: disabled transports emit transport-debug tags and skip per-NS-IP in-domain checks.
  - `resolver.defaults.parallel`: parallel in-domain NS-IP query fanout.
...
| Tag | Emitted when |
| --- | --- |
| `IPV4_DISABLED` | IPv4 in-domain per-NS-IP check is skipped because IPv4 is disabled. |
...
- Differences (Upstream vs Gonemaster):
  - Upstream: special-procedure text is generic about disabled transports. Gonemaster: explicitly emits `IPV4_DISABLED`/`IPV6_DISABLED` only in the in-domain per-NS-IP branch.
  - Upstream: describes recursive sibling/not-in-domain branch as DNS lookup followed by CNAME check. Gonemaster: does that CNAME check, but does not emit `NO_RESPONSE` or `UNEXPECTED_RCODE` in that branch.
...
- In-domain checks are performed against all unique `name/ip` entries from delegation+child sets, not only the tested NS name's own addresses.

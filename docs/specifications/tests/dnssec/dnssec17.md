...
- Differences (Upstream vs Gonemaster):
  - Upstream: describes this testcase as producing no output when no CDNSKEY is found. Gonemaster: emits only testcase boundary/debug tags in that case (`TEST_CASE_START`, `TEST_CASE_END`, and possibly transport-disabled tags).
  - Upstream: signature checks are described at RRset/signature level. Gonemaster: several checks (`DS17_DNSKEY_NOT_SIGNED_BY_CDNSKEY`, `DS17_CDNSKEY_NOT_SIGNED_BY_CDNSKEY`) are implemented as keytag-presence checks in RRSIG sets, not full per-signature validation.

...
- Differences (Upstream vs Gonemaster):
  - Upstream: says input is nameserver IP set. Gonemaster: iterates raw `AllNameservers` output, but aggregate `addresses` values are sorted and deduplicated by IP.

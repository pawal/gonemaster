...
5. For each nameserver (parallelized, input-order merged logs):
   ...
   - Query 1 (support and well-formedness): send a UDP-pinned SOA query for the zone name carrying the COOKIE option with only the 8-byte Client Cookie. UDP is pinned because TCP return-routability lets a server skip cookie processing.
     ...
     - `RCODE != NOERROR`: no cookie verdict (RCODE anomalies are graded by basic/N16).
   ...
   - Query 2 (round-trip): re-query with the full cookie returned by query 1.
...
7. Emit `N17_NO_COOKIE` with sorted unique `servers` when non-empty.
8. Emit `N17_COOKIE_ROUNDTRIP_OK` with sorted unique `servers` when non-empty.
9. Emit `N17_COOKIE_CLIENT_ONLY` with sorted unique `servers` when non-empty.
10. Emit `N17_COOKIE_MALFORMED` per `cookie_bytes` value with sorted unique `servers` when non-empty.
11. Emit `N17_COOKIE_SELF_REJECT` with sorted unique `servers` when non-empty.
...
- A FORMERR to a valid 8-byte Client Cookie is itself a server defect (RFC 7873 5.2.2), but v1 leaves RCODE anomalies to basic/N16 and adds no dedicated tag.

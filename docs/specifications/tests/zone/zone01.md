...
2. Query each nameserver from [`ZoneNameservers`](../../nameserver-resolution.md#zonenameservers) for `SOA` at the child zone apex.
...
6. For each non-`localhost` and non-dot MNAME:
   - If MNAME is not in [`z.ApexNSNames`](../../nameserver-resolution.md#apexnsnames) child NS name set, emit `Z01_MNAME_NOT_IN_NS_LIST`.

...
2. Collect nameservers from `AllNameservers`, deduplicate probing list by IP address, and keep a name-to-IP grouping view for later reporting.
...
{{% expand "Show diagram" %}}
```
ns list = AllNameservers; dedupe by IP (uniqueServersByIP)

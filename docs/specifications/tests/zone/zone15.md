...
- CAA published at a top-level domain apex is consulted for every name in that TLD, so a forbidding or malformed record there affects the entire subtree. Content validation runs there exactly as for any other zone; only the absence tag differs, because neither the root nor a TLD can hold a publicly trusted certificate for its own name.

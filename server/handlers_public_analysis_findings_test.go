...
func (f *analysisFixture) seedGraduatedRun(domainName string, finishedAt time.Time, entries []engine.LogEntry, timings ...NameserverTiming) Run {
	return f.seedGraduatedRunInBatch(f.batchID, domainName, finishedAt, entries, timings...)
}
...
func (f *analysisFixture) seedGraduatedRunInBatch(batchID, domainName string, finishedAt time.Time, entries []engine.LogEntry, timings ...NameserverTiming) Run {
	f.t.Helper()

...
func TestHotCacheHeapGrowthWithForcedGC(t *testing.T) {
	...
	synctest.Test(t, func(t *testing.T) {
		...
		for job := range totalJobs {
			...
			// Sleep so TTLs have a chance to expire regularly. It runs after
			// the checkpoint: before it, the probe lease prunes everything and
			// always reads a warmed base of zero.
			if job%10 == 9 {
		...
		// The warmed base is the retention path this test is about: it has to
		// stay warm, and it must not grow with the job count.
		if len(checkpoints) >= 2 {
			first, last := checkpoints[0], checkpoints[len(checkpoints)-1]
			if first.baseAddrs == 0 || last.baseAddrs == 0 {
				t.Fatalf("warmed base empty (first=%d last=%d): nothing is being retained to leak",
					first.baseAddrs, last.baseAddrs)
			}
			if last.baseAddrs > 2*first.baseAddrs {
				t.Fatalf("warmed base grew from %d to %d addresses over %d jobs",
					first.baseAddrs, last.baseAddrs, last.job)
			}
		}

...
func runSoak(domainsPath string, domains []string, domainsSHA string, outPath string, backend string, sqliteDSN string, workers int, maxConcurrentJobs int, rounds int, warmupRounds int, sampleSeconds int, pollSeconds int, batchTimeoutSeconds int, profilePath string, minLevel string) (soakReport, error) {
	...
	report.Methodsv2CacheEntriesStart = methodsv2.CacheEntryCount()
	...
	for round := 1; round <= rounds; round++ {
		...
		log.Printf("round %d/%d complete: batch=%s wall=%.3fs terminal=%d/%d cache_entries=%d", round, rounds, batch.BatchID, batch.WallSeconds, completed, summary.Total, methodsv2.CacheEntryCount())

		if round == warmupRounds {
			cp, err := takeGCCheckpoint("post_warmup", round, started)
			if err != nil {
				return report, fmt.Errorf("post-warmup checkpoint: %w", err)
			}
			report.Checkpoints = append(report.Checkpoints, cp)
			log.Printf("post-warmup checkpoint: rss=%dKB heap_alloc=%d heap_inuse=%d cache_entries=%d", cp.RSSKB, cp.HeapAllocBytes, cp.HeapInuseBytes, cp.Methodsv2CacheEntries)
		}
		if round == rounds {
			cp, err := takeGCCheckpoint("late_run", round, started)
			if err != nil {
				return report, fmt.Errorf("late-run checkpoint: %w", err)
			}
			report.Checkpoints = append(report.Checkpoints, cp)
			log.Printf("late-run checkpoint: rss=%dKB heap_alloc=%d heap_inuse=%d cache_entries=%d", cp.RSSKB, cp.HeapAllocBytes, cp.HeapInuseBytes, cp.Methodsv2CacheEntries)
		}
	}
	...
	report.Methodsv2CacheEntriesEnd = methodsv2.CacheEntryCount()
	peakMu.Lock()
...
func takeGCCheckpoint(label string, round int, started time.Time) (gcCheckpoint, error) {
	...
	return gcCheckpoint{
		...
		Methodsv2CacheEntries: methodsv2.CacheEntryCount(),
	}, nil
}

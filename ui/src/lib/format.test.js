...
describe("formatAgeShort", () => {
  ...
  it.each([
    ["an empty timestamp", "", ""],
    ["an unparsable timestamp", "garbage", ""],
    ["seconds", "2026-01-02T11:59:15Z", "45s"],
    ["minutes", "2026-01-02T11:48:00Z", "12m"],
    ["hours", "2026-01-02T09:00:00Z", "3h"],
    ["days", "2025-12-25T12:00:00Z", "8d"],
    ["a timestamp in the future", "2026-01-02T12:00:30Z", "0s"]
  ])("renders %s as %j", (_case, ts, want) => {
    expect(formatAgeShort(ts, now)).toBe(want);
  });
});
